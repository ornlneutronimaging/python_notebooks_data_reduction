...
class NormalizationWhiteBeamAtVenus:
    ...
    def initialize(self, working_dir=None, debug=False):
        LOG_PATH = "/SNS/VENUS/shared/log/"
        file_name = f"normalization_white_beam_at_venus"
        ...
        display(HTML(f"<span style='color:green'>log book created: {log_file_name}</span>"))
        
    # def __new__(cls, *args, **kwargs):
    #     logging.info(f"Creating instance of {cls.__name__}")

    def __init__(self, system=None, debug=False):

        self.working_dir = f"/SNS/{system.System.get_instrument_selected()}/IPTS-{system.System.get_ipts_selected()}"
        
        self.initialize(working_dir=self.working_dir, debug=debug)

        display(HTML("<span style='color:blue; font-size:16px'>Select detector type</span>"))
        self.detector_type_widget = widgets.Dropdown(
            options=[DetectorType.ikonxl, DetectorType.qhy600],
    ...
    def setup_paths(self):
        ...
        self.folder_path.shared = Path("/") / _facility / self.instrument / str(ipts) / "shared"
        self.folder_path.ipts = Path("/") / _facility / self.instrument / str(ipts)
        ...
        notebook_logging.info(f"nexus folder: {self.folder_path.nexus}")
